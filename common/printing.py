print_formats = ['mkc', 'mkb', 'summit', 'none', 'mkh','fc','mmr']
...
async def printfc(self, ctx, fc_list, roomnum=0):
    ...
    for fc in fc_list:
        ...
        if player is not None and player.mmr is not None:
            mmr_results.append(f"FC {fc.strip()}: {player.mmr}")
        else:
            mmr_results.append(f"MMR for FC {fc.strip()} not found.")

    if roomnum > 0:
        await ctx.send(mmr_results[roomnum-1])
    else:
        await ctx.send("\n".join(mmr_results))
...
async def printmmr(ctx, rooms, roomnum=0):
    async def playerStr(player):
        if player.canHost:
            host = "★進"
        else:
            host = ""
        mmr = await get_mmr_from_fc(player.fc)
        return f"{player.fc} is ({mmr})"


    def teamStr(team):
        return "".join([player.fc for player in team.players])
...
async def printRooms(ctx, print_format, rooms, roomNum):
    ...
    elif print_format == "fc":
        await printfc(ctx, rooms, roomNum)
    elif print_format == "mmr":
        await printmmr(ctx, rooms, roomNum)     
    else:
